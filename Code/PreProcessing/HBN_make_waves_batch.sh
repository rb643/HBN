	...
	#for subject in sub-NDARAB458VK9 ; do
  		freesurfer_dir=${dataDir}/${site}/derivatives/${subject}/T1w_HCP_run-01/freesurfer
    		...
    		if [ -e ${freesurfer_dir}/mri/brain.mgz ];then
					...
					sbatch --account=BETHLEHEM-SL2-CPU --partition=skylake-himem --output=${dataDir}/logs/rs/${subject}_${site}_rslog.log --nodes=1 --ntasks=1 --cpus-per-task=1 --time=04:00:00 --mem=48000 HBN_make_waves_nt.sh ${subject} ${site} ${rs_file} ${freesurfer_dir} ${t1w_file} ${tpatt}
				fi
	done
