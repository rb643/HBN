...
site=Site-CBIC
	...
	#for subject in sub-NDARAB458VK9 ; do
  		freesurfer_dir=${dataDir}/${site}/derivatives/${subject}/T1w_HCP_run-01/freesurfer
			rs_file=${dataDir}/${site}/${subject}/func/${subject}_task-movieDM_bold.nii.gz
    		...
    		if [ -e ${freesurfer_dir}/mri/brain.mgz ] && [ -e ${rs_file} ]
				then
					...
					sbatch --account=BETHLEHEM-SL2-CPU --partition=skylake-himem --output=${dataDir}/logs/movie/${subject}_${site}_movielog.log --nodes=1 --ntasks=1 --cpus-per-task=1 --time=04:00:00 --mem=48000 HBN_make_waves_movie2.sh ${subject} ${site} ${rs_file} ${freesurfer_dir} ${t1w_file} ${tpatt}
				fi
	done
