...
if options.tds and options.wds:
	...
	sys.exit()
if (options.SP or options.EDOF or options.TW or options.window_init or options.threshold) and not options.wds:
	print "Please specify --wds with other Wavelet Despiking options. Exiting."
	sys.exit()
if options.TW and not options.EDOF:
	print "WARNING: You have not specified --EDOF or --SP with --TW. This combination is mandatory. Computing EDOF maps and SP anyway."
...
# Parse Wavelet Despike window options
if options.TW:
	ntp=float(os.popen('3dinfo -nv %s%s' % (prefix,isf)).readlines()[0].strip())
	window_init=float(options.window_init)
	if ntp<window_init:
		window_init=ntp/2
...
if options.anat!='':
	if options.rcsf or options.rall:
		tempdir="$HOME/matlab/fmri_spt/templates"
...
if options.anat!='':
	sl.append("cp %s/%s* ." % (startdir,dsprefix(nsmprage)))
...
#		atnsmprage = "%s_at.nii.gz" % (dsprefix(nsmprage))
		if options.qwarp:
			...
			sl.append("3dQwarp -iwarp -overwrite -resample -duplo -useweight -blur 2 2 -workhard -base %s/standard/%s -prefix ./%snl.nii.gz -source ./%su.nii.gz" % (tempdir,ssstr,dsprefix(atnsmprage),dsprefix(atnsmprage)) )
			sl.append("3dcopy ./%snl.nii.gz %s/" % (dsprefix(atnsmprage),startdir) )
			sl.append("3dcopy ./%snl_WARP.nii.gz %s/" % (dsprefix(atnsmprage),startdir) )
			sl.append("fi")
	...
	sl.append("3dAllineate -weight_frac 1.0 -VERB -warp aff -weight eBmask_pve_0.nii.gz -lpa -base eBbase.nii.gz -master SOURCE -source %s -prefix ./%s_al -1Dmatrix_save %s_al_mat %s" % (alnsmprage, anatprefix,anatprefix,align_args))
...
if options.anat!='':
	if options.qwarp:
		...
		sl.append("3dNwarpApply -overwrite -nwarp './%snl_WARP.nii.gz %s_wmat.aff12.1D' -master sstemp.nii.gz -dxyz ${voxsize} -source _eBvrmask.nii.gz -interp NN -prefix ./_eBvrmask.nii.gz" % \
		...
		sl.append("3dNwarpApply -nwarp './%snl_WARP.nii.gz %s_wmat.aff12.1D' -master eBvrmask.nii.gz -source %s_ts+orig -interp %s -prefix ./%s_vr%s " % \
...
if options.wds:
	if options.SP and not options.EDOF and not options.TW:
		sl.append("matlab -nodisplay -r \"WaveletDespike('%s_sm%s','%s','SP',1,'EDOF',0,'LimitRAM',22,'threshold',%s);quit\" " % (dsin,osf,dsin,float(options.threshold)))
	elif options.EDOF and not options.SP and not options.TW:
		sl.append("matlab -nodisplay -r \"WaveletDespike('%s_sm%s','%s','SP',0,'EDOF',1,'LimitRAM',22,'threshold',%s);quit\" " % (dsin,osf,dsin,float(options.threshold)))
		if options.ss:
			if ssstr=="MNI152_T1_1mm+tlrc" or ssstr=="MNI_caez_N27+tlrc":
				sl.append("3drefit -space MNI %s_EDOF%s" % (dsin,osf))
			else:
				sl.append("3drefit -space TLRC %s_EDOF%s" % (dsin,osf))
	elif options.TW:
		sl.append("matlab -nodisplay -r \"WaveletDespike('%s_sm%s','%s','SP',1,'EDOF',1,'LimitRAM',22,'TimeWindow',1,'TR',%s,'window',%s,'threshold',%s);quit\" " % \
		(dsin,osf,dsin,options.TR,window_init,float(options.threshold)))
		if options.ss:
			if ssstr=="MNI152_T1_1mm+tlrc" or ssstr=="MNI_caez_N27+tlrc":
				sl.append("3drefit -space MNI %s_EDOF%s" % (dsin,osf))
			else:
				sl.append("3drefit -space TLRC %s_EDOF%s" % (dsin,osf))
	elif options.SP and options.EDOF and not options.TW:
		sl.append("matlab -nodisplay -r \"WaveletDespike('%s_sm%s','%s','SP',1,'EDOF',1,'LimitRAM',22,'threshold',%s);quit\" " % (dsin,osf,dsin,float(options.threshold)))
		if options.ss:
			if ssstr=="MNI152_T1_1mm+tlrc" or ssstr=="MNI_caez_N27+tlrc":
				sl.append("3drefit -space MNI %s_EDOF%s" % (dsin,osf))
			else:
		...
		sl.append("matlab -nodisplay -r \"WaveletDespike('%s_sm%s','%s','SP',0,'EDOF',0,'LimitRAM',22,'threshold',%s);quit\" " % (dsin,osf,dsin,float(options.threshold)))
...
if options.EDOF:
	sl.append("cp %s_EDOF.nii.gz %s" % (prefix,startdir))
